...
void ForEachPciDevice(
  ...
  if ((header_type & 0x80) == 0) /* single pci host controller */
    ...
    for (function = 0; function < 8; function++) {
      if (Read16BitsFromPciConfig(0, 0, function, kPciHdrVendorId) ==
          0xFFFF) /* reference said != but that's funny? */
        break;
...
void InitializePci() {
  ForEachPciDevice([](uint8 base_class, uint8 sub_class, uint8 prog_if,
...
void ForEachPciDeviceThatMatchesQuery(
    int16 base_class, int16 sub_class, int16 prog_if, int32 vendor_id,
  ...
  for (const PciDevice& device : devices) {
    if ((base_class == -1 || base_class == device.base_class) &&
      ...
      on_each_device(device.base_class, device.sub_class, device.prog_if,
                     device.vendor_id, device.device_id, device.bus,
