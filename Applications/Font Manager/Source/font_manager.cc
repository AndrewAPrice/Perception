...
void PopulateFcPatternFromFontStyle(const FontStyle& style, FcPattern& pattern) {
  FcPatternAddInteger(&pattern, FC_WEIGHT,
                      GetOrDefault(kFontWeightToFcInt, style.GetWeight(),
                                   FC_WEIGHT_REGULAR));
  FcPatternAddInteger(
      &pattern, FC_WIDTH,
...
StatusOr<Permebuf<FM::MatchFontResponse>> FontManager::HandleMatchFont(
  ...
  response->SetPath(resolved_filename);
