...
struct OboeImpl {
  static void SynthesizeBlock(double freq, double t_start, double dt,
                              ...
                              size_t count) {
    // Maximum number of overtone harmonics calculated for oboe.
    ...
    if (freq != cache.freq) {
      ...
      for (int h = 1; h <= kMaxHarmonics; ++h) {
        ...
        double reed_formant = FormantResonance(h_freq, 1300.0, 300.0, 2.0);
        double hf_damp = HighFrequencyDamping(h_freq, 5000.0);

        double amp = (1.0 / std::pow(h, 0.9)) * reed_formant * hf_damp;
        cache.weights[cache.num_harmonics++] = amp;
      }
    }

    SynthesizeVibratoAdditiveBlock(freq, t_start, dt, duration_seconds, 5.5,
                                   0.0045, 0.1, 0.2, cache.weights,
...
#pragma GCC ivdep
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      out_samples[i] *= env * 0.28;
    }
  }
};

auto _ = RegisterInstrumentSpec<OboeImpl>("Oboe", "Winds & Brass", 0.14, false,
                                          5.0, 23);
