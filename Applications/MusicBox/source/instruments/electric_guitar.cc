...
#pragma GCC ivdep
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      double decay = std::exp(-1.2 * t / duration_seconds);
      double env = attack * decay;
      double drive = std::clamp(out_samples[i] * 3.0, -1.0, 1.0);
      out_samples[i] = env * drive * 0.6;
    }
  }
};

auto _ = RegisterInstrumentSpec<ElectricGuitarImpl>(
    "Electric Guitar", "Strings & Plucked", 0.15, false, 3.5, 10);
