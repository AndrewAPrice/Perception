...
#pragma GCC ivdep
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      double decay = std::exp(-2.8 * t / duration_seconds);
      double env = attack * decay;
      double phase = std::fmod(freq * t, 1.0);
      double pulse = (phase < 0.15) ? 1.0 : -1.0;
      out_samples[i] = env * pulse * 0.6;
    }
  }
};

auto _ = RegisterInstrumentSpec<ClavinetImpl>(
    "Clavinet", "Keyboards", 0.08, false, 2.0, 3);
