...
void EnableProfiling(processes::Process* process);

// Finishes profiling and outputs the data, passing the process that requested
// it.
void DisableAndOutputProfiling(processes::Process* process);

// Notifies profiler that a process has exited.
void NotifyProfilerThatProcessExited(processes::Process* process);
