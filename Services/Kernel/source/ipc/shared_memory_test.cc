...
TEST(SharedMemoryEventsTest) {
  ...
  SharedMemoryInProcess* shm_p1 =
      CreateAndMapSharedMemoryBlockIntoProcess(p1, 2, kSmJoinersCanWrite, 0);
  ASSERT(shm_p1 != nullptr, true);
...
TEST(SharedMemoryManagerDirectMethodsTest) {
  InitializeObjectPools();
  InitializeProcesses();
  InitializeVirtualAllocator();
  SharedMemoryManager::Get().Initialize();

  Process* p1 = CreateTestProcess("ProcessShmDirect");
  ASSERT(p1 != nullptr, true);

  SharedMemoryInProcess* shm_p1 =
      SharedMemoryManager::Get().CreateAndMap(p1, 2, kSmJoinersCanWrite, 0);
  ASSERT(shm_p1 != nullptr, true);
  ASSERT(shm_p1->mapped_pages, (size_t)2);
  ASSERT(SharedMemoryManager::Get().GetAllocatedBytes(), (size_t)(2 * kPageSize));

  SharedMemoryManager::Get().Leave(p1, shm_p1->shared_memory->id);
}
