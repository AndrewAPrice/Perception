...
[GLOBAL reschedule_ipi_interrupt]
[GLOBAL tlb_shootdown_ipi_interrupt]
...
reschedule_ipi_interrupt:
	push 17
	jmp irq_common_stub

tlb_shootdown_ipi_interrupt:
	push 18
	jmp irq_common_stub

irq_common_stub:
	; If interrupted from user mode (cs & 3 != 0), swap to kernel GS.
	test qword [rsp + 16], 3
	jz .irq_from_kernel
	swapgs
.irq_from_kernel:
...
JumpIntoThread:
    ...
    ; Idle loop: halt on interrupt stack.
    mov rsp, [gs:24]
    sti
.idle_loop:
    hlt
...
.jump_to_thread:
 ...
 .jump_over_post_handler_profiling:
	...
	pop rdi
	pop rbp
	; If returning to user mode (cs & 3 != 0), restore user GS before iretq.
	test qword [rsp + 8], 3
	jz .skip_swapgs
	swapgs
.skip_swapgs:
