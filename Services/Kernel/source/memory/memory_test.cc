...
TEST(MemoryIsKernelAddressTest) {
  ...
  ASSERT(IsKernelAddress(kVirtualMemoryOffset), true); // Kernel address start
...
TEST(MemoryCopyKernelToProcessTest) {
  ...
  ASSERT(phys_page != kOutOfMemory, true);
...
TEST(MemoryZeroProcessMemoryTest) {
  InitializeObjectPools();
  InitializeProcesses();
  InitializeVirtualAllocator();
  ...
  size_t dest_virtual_addr = 0x800000;
  size_t phys_page =
      p1->virtual_address_space.GetOrCreateVirtualPage(dest_virtual_addr);
  ASSERT(phys_page != kOutOfMemory, true);
  ...
  bool success =
      ZeroProcessMemory(dest_virtual_addr, dest_virtual_addr + 32, p1);
  ASSERT(success, true);

  for (size_t i = 0; i < 32; i++) {
    ASSERT((int)dest_data[i], 0);
  }
  ASSERT((int)dest_data[32], 0x5A);
