...
TEST(ElfLoaderLoadProcessTest) {
  ...
  char query_name[kProcessNameLength + 1];
  common::CopyString("my_process", kProcessNameLength, 10, query_name);
  ProcessRef process = FindNextProcessWithName(query_name, 0);
  ...
  ASSERT(process->threads.count(), (size_t)1);
