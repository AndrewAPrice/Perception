...
Frame* Frame::GetRootFrame() { return root_frame; }

Frame* Frame::GetFrameBeingDragged() { return dragging_frame; }
  ...
  if (SplitFrame.is_split_vertically_) {
    ...
    if (screen_y > y_ && y_ < screen_y + height_ - SPLIT_BORDER_WIDTH &&
        ...
        SplitFrame.child_b_->IsValidSize(
            width_, y_ + height_ - screen_y - SPLIT_BORDER_WIDTH)) {
      SetHighlighter(x_, screen_y, x_ + width_, screen_y + SPLIT_BORDER_WIDTH);
    } else {
      DisableHighlighter();
    }
  } else {
    ...
    if (screen_x > x_ && x_ < screen_x + width_ - SPLIT_BORDER_WIDTH &&
      ...
      SetHighlighter(screen_x, y_, screen_x + SPLIT_BORDER_WIDTH, y_ + height_);
    } else {
      DisableHighlighter();
    }
  }
...
void Frame::DroppedAt(int screen_x, int screen_y) {
  ...
  if (SplitFrame.is_split_vertically_) {
    ...
    if (screen_y > y_ && y_ < screen_y + height_ - SPLIT_BORDER_WIDTH &&
        ...
        SplitFrame.child_b_->IsValidSize(
            width_, y_ + height_ - screen_y - SPLIT_BORDER_WIDTH)) {
      SplitFrame.split_percent_ = (float)(screen_y - y_) / (float)height_;
      UpdateFrame(true);
    }
  } else {
    screen_x -= frame_dragging_offset;
    if (screen_x > x_ && x_ < screen_x + width_ - SPLIT_BORDER_WIDTH &&
        ...
        SplitFrame.child_b_->IsValidSize(
            x_ + width_ - screen_x - SPLIT_BORDER_WIDTH, height_)) {
      SplitFrame.split_percent_ = (float)(screen_x - x_) / (float)width_;
      UpdateFrame(true);
    }
  }
}

Frame* Frame::GetDropFrame(const Window& window, int mouse_x, int mouse_y,
  ...
  while (current_frame != nullptr) {
    ...
    if (current_frame->is_split_frame_) {
      if (current_frame->SplitFrame.is_split_vertically_) {
        if (mouse_y <
            current_frame->y_ + current_frame->SplitFrame.split_point_)
        ...
        // in the new frame?
        if (mouse_x <
            current_frame->x_ + current_frame->SplitFrame.split_point_) {
...
void Frame::DropInWindow(Window& window, int mouse_x, int mouse_y) {
  int drop_min_x, drop_min_y, drop_max_x, drop_max_y;
  Frame* drop_frame = Frame::GetDropFrame(window, mouse_x, mouse_y, drop_min_x,
                                          drop_min_y, drop_max_x, drop_max_y);
  if (!drop_frame) {
    // No where to drop us.
    return;
  }

  if (drop_min_x == drop_frame->x_ && drop_min_y == drop_frame->y_ &&
    ...
    child_b->x_ = drop_frame->x_ + drop_frame->width_ / 2 + SPLIT_BORDER_WIDTH;
    ...
    child_b->DockFrame.first_window_ = drop_frame->DockFrame.first_window_;
    child_b->DockFrame.last_window_ = drop_frame->DockFrame.last_window_;
    child_b->DockFrame.focused_window_ = drop_frame->DockFrame.focused_window_;
    ...
    child_a->height_ = drop_frame->height_;
    child_a->is_split_frame_ = false;
    child_a->DockFrame.first_window_ = drop_frame->DockFrame.first_window_;
    child_a->DockFrame.last_window_ = drop_frame->DockFrame.last_window_;
    child_a->DockFrame.focused_window_ = drop_frame->DockFrame.focused_window_;
    ...
    child_b->y_ = drop_frame->y_ + drop_frame->height_ / 2 + SPLIT_BORDER_WIDTH;
    child_b->parent_ = drop_frame;
...
void Frame::UpdateFrame(bool resized, Window* window_to_update) {
  if (is_split_frame_) {
    ...
    if (!SplitFrame
             .child_a_) /* child a closed, promote child b to my position */
      replace_me = SplitFrame.child_b_;
    else if (!SplitFrame.child_b_) /* child b closed, promote child a to my
                                      position */
    ...
    if (resized) {
      ...
      if (SplitFrame.is_split_vertically_) {
        ...
        SplitFrame.child_b_->height_ =
            height_ - split_point - SPLIT_BORDER_WIDTH;
    ...
    new_client_height =
        new_title_height > height_ ? 0 : height_ - new_title_height;
  ...
  if (is_split_frame_) {
    ...
    if (SplitFrame.is_split_vertically_) {
      if (screen_y < y_ + SplitFrame.split_point_) {
        ...
        SplitFrame.child_a_->MouseEvent(screen_x, screen_y, button,
                                        is_button_down);
      } else if (screen_y < y_ + SplitFrame.split_point_ + SPLIT_BORDER_WIDTH) {
        // Split point.
        if (button == MouseButton::Left && is_button_down) {
          // Start dragging the split point.
          dragging_frame = this;
          frame_dragging_offset =
              screen_y - SplitFrame.split_point_ + SPLIT_BORDER_WIDTH;
        }
        Window::MouseNotHoveringOverWindowContents();
        return;
      } else {
        // Clicked the bottom frame.
        SplitFrame.child_b_->MouseEvent(screen_x, screen_y, button,
                                        is_button_down);
      }
    } else {
      if (screen_x < x_ + SplitFrame.split_point_) {
        ...
        SplitFrame.child_a_->MouseEvent(screen_x, screen_y, button,
                                        is_button_down);
      } else if (screen_x < x_ + SplitFrame.split_point_ + SPLIT_BORDER_WIDTH) {
        // Split point.
        if (button == MouseButton::Left && is_button_down) {
          // Start dragging the split point.
          dragging_frame = this;
          frame_dragging_offset =
              screen_x - SplitFrame.split_point_ + SPLIT_BORDER_WIDTH;
        }
        Window::MouseNotHoveringOverWindowContents();
        return;
      } else {
      ...
      while (window) {
        ...
        if (screen_y < next_title_y &&
          ...
          if (button == MouseButton::Left && is_button_down) {
            ...
            window->HandleTabClick(screen_x - title_x, title_x,
                                   next_title_y - WINDOW_TITLE_HEIGHT - 1);
...
void Frame::Draw(int min_x, int min_y, int max_x, int max_y) {
  ...
  if (x_ >= max_x || y_ >= max_y || x_ + width_ < min_x ||
      y_ + height_ < min_y) {
    return;
  }
  ...
  if (is_split_frame_) {
    ...
    if (SplitFrame.is_split_vertically_) {
      ...
      if (y + 1 >= min_y && y <= max_y) {
        ...
        DrawSolidColor(std::max(x_, min_x), std::max(y, min_y),
                       std::min(x_ + width_, max_x), std::min(y + 2, max_y),
                       SPLIT_BORDER_COLOR);
      }
    } else {
      ...
      if (x + 1 >= min_x && x <= max_x) {
        // The middle bar in the draw region.
        DrawSolidColor(std::max(x, min_x), std::max(y_, min_y),
                       std::min(x + 2, max_x), std::min(y_ + height_, max_y),
                       SPLIT_BORDER_COLOR);
      }
    }
    ...
    if (min_y < y_ + DockFrame.title_height_) {
      ...
      while (w) {
        ...
        Window::DrawHeaderBackground(x, y + 1, w->title_width_,
                                     ...
                                     : w == DockFrame.focused_window_
                                         ? UNFOCUSED_WINDOW_COLOUR
      ...
      CopyTexture(std::max(x_, min_x), std::max(y, min_y), std::min(x, max_x),
      ...
      /* draw bottom border */
      DrawSolidColor(
          std::max(x_, min_x), std::max(y + WINDOW_TITLE_HEIGHT + 1, min_y),
          std::min(x_ + width_, max_x),
          std::min(y + WINDOW_TITLE_HEIGHT + 2, max_y), WINDOW_BORDER_COLOUR);
    }

    // Draw the contents of the focused window.
    DockFrame.focused_window_->DrawWindowContents(
        x_, y_ + DockFrame.title_height_, min_x, min_y, max_x, max_y);
  }
}
...
int Frame::LongestWindowTitleWidth() {
  if (is_split_frame_) return 0;

  int longest_window_title_width = 0;
  Window* window = DockFrame.first_window_;
  while (window != nullptr) {
    longest_window_title_width =
        std::max(longest_window_title_width, window->title_width_);
    window = window->next_;
  }
...
bool Frame::IsValidSize(int width, int height) {
  if (is_split_frame_) {
    ...
    if (height <= WINDOW_MINIMUM_HEIGHT) return false;
