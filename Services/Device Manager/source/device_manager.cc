...
DeviceManager::HandleQueryPciDevices(
    ProcessId sender, const DM::QueryPciDevicesRequest& request) {
  Permebuf<DM::QueryPciDevicesResponse> response;
  PermebufListOf<PciDevice> last_device;

  ForEachPciDeviceThatMatchesQuery(
      request.GetBaseClass(), request.GetSubClass(), request.GetProgIf(),
      request.GetVendor(), request.GetDeviceId(), request.GetBus(),
      request.GetSlot(), request.GetFunction(),
