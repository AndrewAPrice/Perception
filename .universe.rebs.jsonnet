{
  ...
  local cpp_compiler = if is_testing then '/usr/bin/clang++' else 'clang',
  local linker = if is_testing then '/usr/bin/clang++' else 'ld.lld',
  local package_type = self.package_type,
  build_commands: {
    // C and C++:
    local c_optimizations =
      if optimization_level == 'optimized' then
        ' -g -Os -fomit-frame-pointer -flto'
    ...
    local c_command_prefix = cpp_compiler + c_optimizations +
                             ...
                             ' -fdata-sections -ffunction-sections -fPIC -fvisibility-inlines-hidden' +
    ...
    s: att_asm,
    S: att_asm,
  } + (if is_testing then {} else {
    // Intel ASM:
    asm: 'nasm -felf64 ${cdefines} ${cincludes} -o ${out} ${in}',
  }),
  local application_linker_optimizations =
    if optimization_level == 'optimized' then ' -O3 --strip-debug'
    else if optimization_level == 'debug' then ' -g'
    else ' -g',
  linker_command:
  ...
  defines: [
    ...
    [],
  dependencies: if is_testing then [] else [
    'libcxx',
    'LLVM Compiler-RT',
    'musl',
    'Perception Core',
