...
size_t FindFreePageRange(size_t pml4, size_t pages) {
  ...
  bool counting = false;
  // How many pages have we counted so far? Terminates when pages ==
  // pages_counted.
  ...
  for (i = pml4_scan_start; i < pml4_scan_end && pages_counted < pages; i++) {
    ...
    if (ptr[i] == 0) {
      ...
      pages_counted +=
          PAGE_TABLE_ENTRIES * PAGE_TABLE_ENTRIES * PAGE_TABLE_ENTRIES;
    } else {
      // There's an entry.
        ...
        if (ptr[j] == 0) {
          ...
          for (k = 0; k < PAGE_TABLE_ENTRIES && pages_counted < pages; k++) {
            if (ptr[k] == 0) {
              ...
              size_t l;
              for (l = 0; l < PAGE_TABLE_ENTRIES && pages_counted < pages;
                   l++) {
                if (ptr[l] == 0) {
                  if (!counting) {
                    counting = true;
  ...
  //                     #### #### #@@@ @@@@ @@!! !!!! !!!+ ++++ ++++ ^^^^ ^^^^
  //                     ^^^^ pml4       pml3       pml2       pml1        flags
  size_t addr = (start_pml4_entry << 39) | (start_pml3_entry << 30) |
                (start_pml2_entry << 21) | (start_pml1_entry << 12);

  // Make the address canonical.
  if (start_pml4_entry >= PAGE_TABLE_ENTRIES / 2) addr += 0xFFFF000000000000;
...
bool MapPhysicalPageToVirtualPage(size_t pml4, size_t virtualaddr,
  ...
  if (created_pml3 && !user_page) {
    // We modified the kernel's PML4. Copy this into every user application's
    // PML4.
    // TODO
  }
...
// Creates a process's virtual address space, returns the PML4. Returns
...
struct SharedMemoryInProcess *MapSharedMemoryIntoProcess(
  ...
  size_t virtual_address =
      FindFreePageRange(process->pml4, shared_memory->size_in_pages);
