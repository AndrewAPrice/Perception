...
double demo_convert_time(struct timeval *tv) {
    double accu;
    /* Combine the value into floating number */
    accu = (double)tv->tv_sec; /* Seconds that have gone by */
    accu *= 1000000.0f; /* One second is 1000x1000x1000 nanoseconds, s -> ms, us, ns */
    accu += (double)tv->tv_usec; /* Nanoseconds that have gone by */
    /* Bring it back into the second range*/
    accu /= 1000000.0f;
    return accu;
}
...
TurkeyVariable test_end(TurkeyVM *vm, void *closure, unsigned int argc, uint64_t types, ...) {
	...
	timeval a, b;		
	TIMESPEC_TO_TIMEVAL(&a, &ts_end);
	TIMESPEC_TO_TIMEVAL(&b, &ts_start);
	
	timeval c; timersub(&a, &b, &c);
	double time = demo_convert_time(&c);
