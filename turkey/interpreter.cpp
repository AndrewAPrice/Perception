#include "hooks.h"
#include "turkey.h"
#include <stdarg.h> /* needed for vaarg intristics */
...
TurkeyVariable turkey_call_function(TurkeyVM *vm, TurkeyFunctionPointer *funcptr, unsigned int argc) {
	...
	if(funcptr->is_native) {
		...
		switch(argc) {
		case 0:
			var = funcptr->data.native.function(vm, funcptr->data.native.closure, argc, 0);
			break;
		case 1: {
			uint64_t type;
			...
			var = funcptr->data.native.function(vm, funcptr->data.native.closure, argc, type, tmp.unsigned_value);
			break; }
		case 2: {
			uint64_t type;
			...
			var = funcptr->data.native.function(vm, funcptr->data.native.closure, argc, type, tmp[0].unsigned_value, tmp[1].unsigned_value);
			break; }
		case 3: {
			uint64_t type;
			...
			var = funcptr->data.native.function(vm, funcptr->data.native.closure, argc, type, tmp[0].unsigned_value, tmp[1].unsigned_value, tmp[2].unsigned_value);
			break; }
		case 4: {
			uint64_t type;
	...
	TurkeyFunction *func = funcptr->data.managed.function;
	...
	// set up closures
	state.closure = funcptr->data.managed.closure;
	if(func->closures > 0) {
		state.closure = turkey_closure_create(vm, funcptr->data.managed.closure, func->closures);
	} else
		state.closure = funcptr->data.managed.closure;
