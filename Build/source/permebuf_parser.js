...
function parseFile(
  ...
  cppIncludeFiles['permebuf/' + getPackageTypeDirectoryName(packageType) + '/' + packageName + '/' + localPath + '.h'] =
      true;
  cppIncludeLiteFiles['permebuf/' + getPackageTypeDirectoryName(packageType) + '/' + packageName + '/' + localPath + '.lite.h'] =
      true;
  ...
  function parsePath() {
    ...
    while (true) {
      if (lexer.peekToken() == '' || lexer.peekToken() == '"' ||
          lexer.peekToken() == '>') {
        addPathPart();
        return path;
      }
  ...
  function parseImport() {
    ...
    return parseFile(
        importedLocalPath, subPackageName, subPackageType, importedFiles,
  ...
  function parseNamespace() {
    ...
    namespace == '';
    while (permebufLexer.isIdentifier(lexer.peekToken()) ||
           lexer.peekToken() == '.') {
  ...
  function parseMessageField(thisMessage) {
    ...
    if (!permebufLexer.isIdentifier(fieldName)) {
      lexer.expectedTokens(
          ['identifier for the message field name'], messageName);
      return false;
    }
  ...
  function parseReserve(thisClass) {
    if (thisClass.classType != ClassType.MESSAGE) {
      console.log(
          '\'reserve\' can only be used in messages, and ' +
          thisClass.fullName + ' in ' + thisClass.fileName +
          ' is not a message.');
      return false;
    }
    ...
    if (fileBeingCompiled) {
      thisClass.reservedFields.push(
          {'name': 'reserve', 'type': type, 'number': parseInt(fieldNumber)});
    }
  ...
  function parseClass(parent) {
    ...
    if (symbolTable[fullClassName]) {
      ...
      console.log(
          fullClassName + ' is already defined in ' +
          symbolTable[fullClassName].fileName);
      return false;
    }
  ...
  while (true) {
    switch (lexer.peekToken()) {
      ...
      case 'namespace':
        if (namespace != '') {
          console.log(
              '\'namespace\' in ' + filename + ' at row ' +
              lexer.getPosition().row + ' column ' + lexer.getPosition().col +
      ...
      default:
        lexer.expectToken(
            ['enum', 'import', 'message', 'namespace', 'oneof', 'end of file'],
            lexer.readToken());
        return false;
    }
  }
