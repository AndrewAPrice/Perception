...
`;
  ...
  let sourceCpp = `#include "perception/fibers.h"
...
`;
  ...
  function getTypeInformation(thisClass, field, subtype) {
    ...
    switch (typeStr) {
      case 'bytes':
        return {'type': FieldType.BYTES, 'sizeInBytes': 0, 'sizeInPointers': 1};
      case 'string':
        return {
      ...
      case 'pointer':
        if (field.name != 'reserve') {
          console.log(
              'Field ' + field.name + '/' + field.number + ' in ' +
              thisClass.fullName +
              ' has type pointer, but this is only valid for reserved fields.');
          return false;
        }
        return {
      ...
      case 'enum':
        if (field.name != 'reserve') {
          console.log(
              'Field ' + field.name + '/' + field.number + ' in ' +
              thisClass.fullName +
              ' has type enum, but this is only valid for reserved fields.');
          return false;
        }
        return {'type': FieldType.ENUM, 'sizeInBytes': 2, 'sizeInPointers': 0};
      case 'oneof':
        if (field.name != 'reserve') {
          console.log(
              'Field ' + field.name + '/' + field.number + ' in ' +
              thisClass.fullName +
              ' has type oneof, but this is only valid for reserved fields.');
          return false;
        }
        return {'type': FieldType.ONEOF, 'sizeInBytes': 2, 'sizeInPointers': 1};
      case 'SharedMemory':
        return {
    ...
    if (field.name == 'reserve') {
      console.log(
          'Reserved field ' + field.number + ' in ' + thisClass.fullName +
          ' has type ' + field.type +
          ' but should be simplified to pointer/enum/oneof.');
      return false;
    }
    ...
    if (typeStr.startsWith('list<') && typeStr.endsWith('>')) {
      const subType =
          getTypeInformation(thisClass, field, typeStr.slice(5, -1));
      if (!subType) {
        return false;
      }
      return {
      ...
      const subType =
          getTypeInformation(thisClass, field, typeStr.slice(6, -1));
      if (!subType) {
        return false;
      }
      return {
    ...
    if (typeStr.startsWith('.')) {
      const typeInformation =
          getTypeInformationForFullTypeName(typeStr.slice(1));
      if (!typeInformation) {
        console.log(
            'Field ' + field.name + '/' + field.number + ' in ' +
      ...
      // Walk up the parent classes and namespaces to see what the type refers
      // to.
      let namePrefix = thisClass.fullName;

      while (true) {
        const typeInformation = getTypeInformationForFullTypeName(
            namePrefix.length > 0 ? namePrefix + '.' + typeStr : typeStr);
        if (typeInformation) {
          return typeInformation;
        }
  ...
  function generateEnum(thisEnum) {
    headerCpp += '\nenum class ' + thisEnum.cppClassName +
        ' : unsigned short {\n' +
        '\tUnknown = 0';
    ...
    for (let i = 0; i < enumFieldKeys.length; i++) {
      ...
      if (usedFieldNumbers[enumField.number]) {
        console.log(
            'Field ' + enumField.name + ' in ' + thisEnum.fullName +
      ...
      if (usedFieldNames[enumField.name]) {
        console.log(
            'Enum ' + thisEnum.fullName + ' has multiple fields labelled ' +
            enumField.name);
        return false;
      }
...
`;
    ...
    // Pretend all 8 bits are used, so the next boolean we encounter will
    // allocate a new byte.
    let bitfieldByteOffset = -1;
    let bitfieldPointerOffset = -1;
    let bitfieldBit = 7;

    // Construct a map of fields by number, so we can loop through them in
    // order.
    ...
    for (let i = 0; i < fieldNames.length; i++) {
      ...
      if (fieldsByNumber[field.number]) {
        console.log(
            'Field ' + field.name + ' has a field number of ' + field.number +
            ' but message ' + thisMessage.fullName +
            ' already has a field with that number.');
        return false;
      }
    ...
    for (let i = 0; i < thisMessage.reservedFields.length; i++) {
      ...
      if (fieldsByNumber[field.number]) {
        console.log(
            'Message ' + thisMessage.fullName +
            ' is trying to reserve the field ' +
            'number ' + field.number +
            ' but the field number is already used.');
        return false;
      }
      fieldsByNumber[field.number] = field;
      if (field.number > maxFieldNumber) {
    ...
    for (let i = 1; i <= maxFieldNumber; i++) {
      if (!fieldsByNumber[i]) {
        console.log(
            'Message ' + thisMessage.fullName +
            ' has a gap in the field numbers. ' +
            'We are missing field ' + i + ', and the highest field number is ' +
            maxFieldNumber +
      ...
      if (typeInformation.type == FieldType.BOOLEAN) {
        // Booleans sizes are handle specially, because they are grouped
        // together into bit field bytes.
        bitfieldBit++;
        if (bitfieldBit == 8) {
          bitfieldBit = 0;
      ...
      if (field.name != 'reserve') {
        switch (typeInformation.type) {
          case FieldType.BOOLEAN:
            ...
            sourceCpp += '\nbool ' + thisMessage.cppClassName + '::Get' +
                field.name + '() const {\n' +
                '\tsize_t address_offset = ';
            if (bitfieldPointerOffset > 0) {
              sourceCpp += '(' + bitfieldPointerOffset +
            ...
            sourceCpp += ' & 1);\n' +
                '}\n' +
                '\nvoid ' + thisMessage.cppClassName + '::Set' + field.name +
                '(bool value) {\n' +
                '\tsize_t address_offset = ';
            if (bitfieldPointerOffset > 0) {
              sourceCpp += '(' + bitfieldPointerOffset +
          ...
          case FieldType.LIST:
            ...
            if (!cppType) {
              console.log(
                  'Something happened parsing type ' + field.type +
                  ' for field ' + field.name + '/' + field.number +
            ...
            sourceCpp += '\nconst ' + fullCppType + ' ' +
                thisMessage.cppClassName + '::Get' + field.name +
                '() const {\n' +
                '\tsize_t address_offset = ';
            if (sizeInPointers > 0) {
            ...
            break;
          case FieldType.ENUM:
            headerCpp += '\t\t' + typeInformation.cppClassName + ' Get' +
                field.name + '() const;\n' +
                '\t\tvoid Set' + field.name + '(const ' +
            ...
            sourceCpp += sizeInBytes + ';\n' +
                ...
                '\t}\n' +
                '\treturn static_cast<' + typeInformation.cppClassName +
                '>(buffer_->Read2Bytes(offset_ + address_offset));\n' +
                '}\n' +
                '\nvoid ' + thisMessage.cppClassName + '::Set' + field.name +
                '(const ' + typeInformation.cppClassName + '& value) {\n' +
                '\tsize_t address_offset = ';
            if (sizeInPointers > 0) {
              sourceCpp += '(' + sizeInPointers +
                  ' << static_cast<size_t>(buffer_->GetAddressSize())) + ';
            ...
            sourceCpp += sizeInBytes + ';\n' +
                ...
                '\nvoid ' + thisMessage.cppClassName + '::Clear' + field.name +
                '() {\n' +
                '\tSet' + field.name + '(' + typeInformation.cppClassName +
                '::Unknown);\n' +
                '}\n';
            break;
          case FieldType.MESSAGE:
            headerCpp += '\t\tconst ' + typeInformation.cppClassName + ' Get' +
                field.name + '() const;\n' +
                '\t\tvoid Set' + field.name + '(' +
                typeInformation.cppClassName + ' value);\n' +
                '\t\t' + typeInformation.cppClassName + ' Mutable' +
                field.name + '();\n' +
                '\t\tbool Has' + field.name + '() const;\n' +
                '\t\tvoid Clear' + field.name + '();\n';

            sourceCpp += '\nconst ' + typeInformation.cppClassName + ' ' +
                thisMessage.cppClassName + '::Get' + field.name +
                '() const {\n' +
                '\tsize_t address_offset = ';
            if (sizeInPointers > 0) {
            ...
            sourceCpp += sizeInBytes + ';\n' +
                ...
                '\t}\n' +
                '\tauto message = buffer_->AllocateMessage<' +
                typeInformation.cppClassName + '>();\n' +
          ...
          case FieldType.ONEOF:
            ...
            break;
          case FieldType.STRING:
            headerCpp += '\t\t::PermebufString Get' + field.name +
                '() const;\n' +
            ...
            sourceCpp += '\n::PermebufString ' + thisMessage.cppClassName +
                '::Get' + field.name + '() const {\n' +
                '\tsize_t address_offset = ';
            if (sizeInPointers > 0) {
              sourceCpp += '(' + sizeInPointers +
                  ' << static_cast<size_t>(buffer_->GetAddressSize())) + ';
          ...
          case FieldType.BYTES:
            if (typeInformation.sizeInPointers == 0) {
              console.log(
                  'Bytes with fixed sizes aren\'t supported in messages. ' +
                  field.type + ' is being used for field ' + field.name + '/' +
                  field.number + ' in message ' + thisMessage.fullName + '.');
              return false;
            }
            headerCpp += '\t\t::PermebufBytes Get' + field.name +
                '() const;\n' +
            ...
            sourceCpp += '\n::PermebufBytes ' + thisMessage.cppClassName +
                '::Get' + field.name + '() const {\n' +
                '\tsize_t address_offset = ';
            if (sizeInPointers > 0) {
              sourceCpp += '(' + sizeInPointers +
          ...
          case FieldType.NUMBER:
            ...
            sourceCpp += sizeInBytes + ';\n' +
                '\tif (address_offset + ' + typeInformation.sizeInBytes +
                ' > size_) {\n' +
                '\t\treturn static_cast<' + typeInformation.cppClassName +
                '>(0);\n' +
                '\t}\n' +
                '\tauto temp_value = buffer_->Read' +
                typeInformation.sizeInBytes +
                (typeInformation.sizeInBytes == 1 ? 'Byte' : 'Bytes') +
                '(offset_ + address_offset);\n' +
                '\treturn *reinterpret_cast<' + typeInformation.cppClassName +
                '*>(&temp_value);\n' +
                '}\n' +
                '\nvoid ' + thisMessage.cppClassName + '::Set' + field.name +
                '(' + typeInformation.cppClassName + ' value) {\n' +
            ...
            sourceCpp += sizeInBytes + ';\n' +
                ...
                '\t\treturn;\n' +
                '\t}\n' +
                '\tbuffer_->Write' + typeInformation.sizeInBytes +
                (typeInformation.sizeInBytes == 1 ? 'Byte' : 'Bytes') +
                '(offset_ + address_offset, *reinterpret_cast<uint' +
                (typeInformation.sizeInBytes * 8) + '_t*>(&value));\n' +
                '}\n' +
                '\nvoid ' + thisMessage.cppClassName + '::Clear' + field.name +
                '() {\n' +
                '\tSet' + field.name + '(0);\n' +
          ...
          case FieldType.SERVICE:
            headerCpp += '\t\t' + typeInformation.cppClassName + ' Get' +
                ...
                typeInformation.cppClassName + '& value);\n' +
                '\t\tvoid Set' + field.name + '(const ' +
                typeInformation.cppClassName + '_Server& value);\n' +
                '\t\tbool Has' + field.name + '() const;\n' +
                '\t\tvoid Clear' + field.name + '();\n';
...
`;
    ...
    for (let i = 0; i < fieldNames.length; i++) {
      ...
      if (fieldsByNumber[field.number]) {
        console.log(
            'Field ' + field.name + ' has a field number of ' + field.number +
            ' but mini-message ' + thisMessage.fullName +
            ' already has a field with that number.');
        return false;
      }
      fieldsByNumber[field.number] = field;
    ...
    for (let i = 0; i < thisMessage.reservedFields.length; i++) {
      ...
      if (fieldsByNumber[field.number]) {
        console.log(
            'Mini-message ' + thisMessage.fullName +
            ' is trying to reserve the field ' +
            'number ' + field.number +
            ' but the field number is already used.');
        return false;
    ...
    for (let i = 1; i <= maxFieldNumber; i++) {
      ...
      if (field.name != 'reserve') {
        switch (typeInformation.type) {
          ...
          case FieldType.MINIMESSAGE:
            console.log(
                'Arrays, lists, messages, one-ofs, strings, and mini-messages aren\'t supported in mini-messages. ' +
                field.type + ' is being used for field ' + field.name + '/' +
                field.number + ' in message ' + thisMessage.fullName + '.');
            return false;
          case FieldType.ENUM:
            ...
            sourceCpp += '\n' + typeInformation.cppClassName + ' ' +
                ...
                typeInformation.cppClassName + '*>(&bytes[address_offset]);\n' +
                '}\n' +
                '\nvoid ' + thisMessage.cppClassName + '::Set' + field.name +
                '(const ' + typeInformation.cppClassName + '& value) {\n' +
                '\tsize_t address_offset = ' + sizeInBytes + ';\n' +
                '\t*reinterpret_cast<' + typeInformation.cppClassName +
                '*>(&bytes[address_offset]) = value;;\n' +
                '}\n' +
                '\nvoid ' + thisMessage.cppClassName + '::Clear' + field.name +
                '() {\n' +
                '\tSet' + field.name + '(' + typeInformation.cppClassName +
          ...
          case FieldType.BYTES:
            if (typeInformation.sizeInPointers == 1) {
              console.log(
                  'Bytes are only supported in mini-messages if they have a fixed size. ' +
                  field.type + ' is being used for field ' + field.name + '/' +
                  field.number + ' in message ' + thisMessage.fullName + '.');
              return false;
            }
            headerCpp += '\t\tconst void* Raw' + field.name + '() const;\n' +
                '\t\tvoid* Raw' + field.name + '();\n' +
                '\t\tsize_t SizeOf ' + field.name + '() const { return ' +
                typeInformation.sizeInBytes + '; }\n';

            sourceCpp += '\nconst void* ' + thisMessage.cppClassName + '::Get' +
                field.name + '() const {\n' +
                '\tsize_t address_offset = ' + sizeInBytes + ';\n' +
          ...
          case FieldType.NUMBER:
            ...
            sourceCpp += '\n' + typeInformation.cppClassName + ' ' +
                ...
                '() const {\n' +
                '\tsize_t address_offset = ' + sizeInBytes + ';\n' +
                '\tauto temp_value = *(uint' +
                (typeInformation.sizeInBytes * 8) +
                '_t*)(&bytes[address_offset]);\n' +
                '\treturn *reinterpret_cast<' + typeInformation.cppClassName +
                '*>(&temp_value);\n' +
                '}\n' +
                '\nvoid ' + thisMessage.cppClassName + '::Set' + field.name +
                '(' + typeInformation.cppClassName + ' value) {\n' +
                '\tsize_t address_offset = ' + sizeInBytes + ';\n' +
                '\t*(uint' + (typeInformation.sizeInBytes * 8) +
                '_t*)(&bytes[address_offset]) = ' +
                '*reinterpret_cast<uint' + (typeInformation.sizeInBytes * 8) +
                '_t*>(&value);\n' +
                '}\n' +
                '\nvoid ' + thisMessage.cppClassName + '::Clear' + field.name +
                '() {\n' +
          ...
          case FieldType.SERVICE:
            ...
            sourceCpp += '\n' + typeInformation.cppClassName + ' ' +
                ...
                '() const {\n' +
                '\treturn ' + typeInformation.cppClassName +
                '(*(::perception::ProcessId*)&bytes[' + sizeInBytes +
                '],*(::perception::MessageId*)&bytes[' + (sizeInBytes + 8) +
                ']);\n' +
                '}\n' +
                '\nvoid ' + thisMessage.cppClassName + '::Set' + field.name +
                '(const ' + typeInformation.cppClassName + '& value) {\n' +
                ...
                '\nbool ' + thisMessage.cppClassName + '::Has' + field.name +
                '() const {\n' +
                '\treturn *(::perception::ProcessId*)(&bytes[' + sizeInBytes +
                ']) != 0;\n' +
                '}\n' +
                '\nvoid ' + thisMessage.cppClassName + '::Clear' + field.name +
                '() {\n' +
...
`;
      ...
      if (sizeInBytes > MAX_MINI_MESSAGE_SIZE) {
        console.log(
            'Mini-message ' + thisMessage.fullName + ' is ' + sizeInBytes +
            ' and the ' +
            'maximum allowed size of a mini-message is ' +
            MAX_MINI_MESSAGE_SIZE + '.');
        return false;
      }
    }
...
bool ${thisService.cppClassName}_Server::DelegateMessage(
    ...
    for (let i = 0; i < fieldNames.length; i++) {
      ...
      if (fieldsByNumber[field.number]) {
        console.log(
            'Function ' + field.name + ' has a function number of ' +
            field.number + ' but service ' + thisService.fullName +
            ' already has a function with that number.');
        return false;
      }
      fieldsByNumber[field.number] = field;
      if (field.number > MAX_SERVICE_FUNCTION_NUMBER) {
        console.log(
            'Function ' + field.name + ' has a function number of ' +
            field.number + ' in service ' + thisService.fullName +
            ', but the maximum allowed function number is ' +
            MAX_SERVICE_FUNCTION_NUMBER + '.');
        return false;
      }

      const requestType =
          getTypeInformation(thisService, field, field.requestType);
      if (!requestType) {
        return false;
      }
      ...
      if (field.responseType != null) {
        responseType =
            getTypeInformation(thisService, field, field.responseType);
        if (!responseType) {
          return false;
        }
      }
      ...
      if (field.isStream) {
        console.log(
            'TODO: Implement code generation for streams in ' +
...
`;
            ...
            console.log(
                'Function ' + field.name + '/' + field.number + ' in service ' +
                thisService.fullName + ' has a response type ' +
                field.responseType +
...
`;
        ...
        default:
          console.log(
              'Function ' + field.name + '/' + field.number + ' in service ' +
              thisService.fullName + ' has a request type ' +
              field.requestType +
              ' that is neither a message nor a mini-message.');
          return false;
      }
...
class ${symbol.cppClassName}_Server;`;
  ...
  writeFile(
      headerCpp,
      packageRootDirectory + 'generated/include/' + generatedFilePath + '.h');
  writeFile(
      sourceCpp,
      packageRootDirectory + 'generated/source/' + generatedFilePath + '.cc');
