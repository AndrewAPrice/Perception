...
const git_repository = 'https://gitlab.gnome.org/GNOME/libxml2.git';

// Check that the third directory exists.
if (!fs.existsSync('third_party')) {
	console.log('Downloading gnome/libxml2');
	// Grab it from github.
	const command = 'git clone ' + git_repository + ' third_party';
	try {
		child_process.execSync(command, {stdio: 'inherit'});
	} catch (exp) {
		console.log('Error downloading gnome/libxml2: ' + exp);
		process.exit(1);
	}
} else {
	console.log('Attempting to update gnome/libxml2');
	// Try to update it.
	const command = 'git pull ' + git_repository;
	try {
		child_process.execSync(command, {cwd: 'third_party', stdio: 'inherit'});
	} catch (exp) {
		console.log('Error updating gnome/libxml2: ' + exp);
		process.exit(1);
	}
}
...
const acceptableExtensions = {'.c':1, '.h':1};
...
copyFilesInDirectory('third_party/include/libxml', 'public/libxml', '.h');
copyFilesInDirectory('third_party/include/private', 'source/private', '.h');
copyFilesInDirectory('third_party', 'source');
copyFile('third_party/include/libxml/xmlversion.h.in', 'public/libxml/xmlversion.h');

const versions = readFromFile('third_party/configure.ac', [
	/m4_define\(\[MAJOR_VERSION\], (\d+)\)/,
	/m4_define\(\[MINOR_VERSION\], (\d+)\)/,
	/m4_define\(\[MICRO_VERSION\], (\d+)\)/]);

replaceInFile('public/libxml/xmlversion.h', [
	['@VERSION@', versions[0][1] +'.'+versions[1][1]+'.' + versions[2][1]],
	['@LIBXML_VERSION_NUMBER@', versions[0][1] * 10000 + versions[1][1] * 100 + versions[2][1]],
