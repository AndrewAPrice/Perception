...
class SK_API SkFontConfigInterface : public SkRefCnt {
 public:
  ...
  struct FontIdentity {
    ...
    bool operator==(const FontIdentity& other) const {
      return fID == other.fID && fTTCIndex == other.fTTCIndex &&
             fString == other.fString && fIsBuffer == other.fIsBuffer &&
             (fBuffer == other.fBuffer && fBuffer && *fBuffer == *other.fBuffer);
    }
    bool operator!=(const FontIdentity& other) const {
      return !(*this == other);
    }
    ...
    uint32_t fID;
    int32_t fTTCIndex;
    // Is the font stored in a shard buffer instead of needing to be loaded from
    // a file?
    bool fIsBuffer;
    SkString fString;
   ...
   */
  virtual bool matchFamilyName(const char familyName[], SkFontStyle requested,
