...
extern "C" int __cxa_atexit(void (function)(void *), void *object, void *dso) {
        // We don't yet support dynamic shared objects, so we can ignore the dso
argument.
        ...
        if (function != nullptr) {
                ...
                while (this_func != nullptr) {
                        if (this_func->function == function) {
                                // Found an exit function that matches what we
want to call. this_func->function(this_func->object);

                                // Remove us from the linked list and iterate to
the next exit function. ExitFunction* next = this_func->next; delete this_func;
