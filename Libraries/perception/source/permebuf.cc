...
PermebufString::PermebufString(PermebufBase* buffer, size_t offset)
    : buffer_(buffer), address_(offset) {}

std::string_view PermebufString::operator->() const { return **this; }
...
std::string_view PermebufString::operator*() const {
  ...
  return std::string_view(
      (const char*)buffer_->GetRawPointer(address_ + bytes, length), length);
}

bool PermebufString::IsEmpty() const { return address_ == 0; }
...
size_t PermebufString::Address() const { return address_; }

void* PermebufString::RawString() const {
  if (address_ == 0) return (void*)kEmptyString.c_str();

  size_t bytes;
  size_t length = buffer_->ReadVariableLengthNumber(address_, bytes);
  if (length == 0) return (void*)kEmptyString.c_str();
  void* raw_string = buffer_->GetRawPointer(address_ + bytes, length);
  if (raw_string == nullptr) return (void*)kEmptyString.c_str();
  return raw_string;
}

PermebufBytes::PermebufBytes(PermebufBase* buffer, size_t offset)
    : buffer_(buffer), address_(offset) {}

void* PermebufBytes::operator->() const { return **this; }

void* PermebufBytes::operator*() const {
  if (address_ == 0) return nullptr;
...
void* PermebufBytes::RawBytes() const { return **this; }
...
PermebufArrayOfStrings::PermebufArrayOfStrings(PermebufBase* buffer,
                                               size_t offset)
    : PermebufArray(buffer, offset) {}
...
void PermebufArrayOfStrings::Set(int index, std::string_view value) {
  ...
  buffer_->WritePointer(string_offset,
...
void PermebufArrayOfBytes::Set(int index, void* value, size_t length) {
  ...
  buffer_->WritePointer(string_offset,
                        buffer_->AllocateBytes(value, length).Address());
}
...
PermebufListOfBooleans::PermebufListOfBooleans(PermebufBase* buffer,
                                               size_t offset)
...
PermebufListOfStrings::PermebufListOfStrings(PermebufBase* buffer,
                                             size_t offset)
...
void PermebufListOfStrings::Set(std::string_view value) {
  ...
  buffer_->WritePointer(GetItemAddress(),
                        buffer_->AllocateString(value).Address());
}
...
size_t PermebufBase::ReadVariableLengthNumber(size_t address,
                                              size_t& bytes) const {
...
void PermebufBase::Write1Byte(size_t address, uint8_t value) {
  ...
  *reinterpret_cast<uint8_t*>(reinterpret_cast<size_t>(start_of_memory_) +
                              address) = value;
}
...
void PermebufBase::Write2Bytes(size_t address, uint16_t value) {
  ...
  *reinterpret_cast<uint16_t*>(reinterpret_cast<size_t>(start_of_memory_) +
                               address) = value;
}
...
void PermebufBase::Write4Bytes(size_t address, uint32_t value) {
  ...
  *reinterpret_cast<uint32_t*>(reinterpret_cast<size_t>(start_of_memory_) +
                               address) = value;
}
...
void PermebufBase::Write8Bytes(size_t address, uint64_t value) {
  ...
  *reinterpret_cast<uint64_t*>(reinterpret_cast<size_t>(start_of_memory_) +
                               address) = value;
}
...
void PermebufBase::WriteVariableLengthNumber(size_t address, size_t value) {
  if (!(value &
        0b1111'1111'1111'1111'1111'1111'1111'1111'1111'1111'1111'1111'1111'1111'1000'0000)) {
    // 7 bit number
    if (!GrowTo(address + 1)) {
      return;
    }
    Write1Byte(address, static_cast<uint8_t>(value << 1));
      ...
      !(value &
        0b1111'1111'1111'1111'1111'1111'1111'1111'1111'1111'1111'1111'1100'0000'0000'0000)) {
    // 14 bit number
    if (!GrowTo(address + 2)) {
      return;
    }
    Write2Bytes(address, 0b0000'0001 + static_cast<uint16_t>(value << 2));
      ...
      !(value &
        0b1111'1111'1111'1111'1111'1111'1111'1111'1111'1111'1110'0000'0000'0000'0000'0000)) {
    // 21 bit number
    if (!GrowTo(address + 3)) {
      return;
    }
    ...
    Write2Bytes(address + 1, static_cast<uint16_t>(value >> 5));
      ...
      !(value &
        0b1111'1111'1111'1111'1111'1111'1111'1111'1111'0000'0000'0000'0000'0000'0000'0000)) {
    // 28 bit number
    if (!GrowTo(address + 4)) {
      return;
    }
    Write4Bytes(address, 0b0000'0111 + static_cast<uint32_t>(value << 4));
      ...
      !(value &
        0b1111'1111'1111'1111'1111'1111'1111'1000'0000'0000'0000'0000'0000'0000'0000'0000)) {
    // 35 bit number
    if (!GrowTo(address + 5)) {
      return;
    }
    ...
    Write4Bytes(address + 1, static_cast<uint32_t>(value >> 3));
      ...
      !(value &
        0b1111'1111'1111'1111'1111'1100'0000'0000'0000'0000'0000'0000'0000'0000'0000'0000)) {
    // 42 bit number
    if (!GrowTo(address + 6)) {
      return;
    }
    ...
    Write4Bytes(address + 2, static_cast<uint32_t>(value >> 10));
      ...
      !(value &
        0b1111'1111'1111'1110'0000'0000'0000'0000'0000'0000'0000'0000'0000'0000'0000'0000)) {
    // 49 bit number
    if (!GrowTo(address + 7)) {
      return;
    }
    ...
    Write4Bytes(address + 3, static_cast<uint32_t>(value >> 17));
      ...
      !(value &
        0b1111'1111'0000'0000'0000'0000'0000'0000'0000'0000'0000'0000'0000'0000'0000'0000)) {
    // 56 bit number
    if (!GrowTo(address + 8)) {
      return;
    }
...
void* PermebufBase::GetRawPointer(size_t address, size_t data_length) {
  ...
  return reinterpret_cast<void*>(reinterpret_cast<size_t>(start_of_memory_) +
                                 address);
}

size_t PermebufBase::Size() const { return size_; }
...
PermebufString PermebufBase::AllocateString(std::string_view value) {
  ...
  size_t string_length_bytes =
      GetBytesNeededForVariableLengthNumber(string_length);
  ...
  memcpy(reinterpret_cast<void*>(reinterpret_cast<size_t>(start_of_memory_) +
                                 string_address + string_length_bytes),
...
PermebufBase::PermebufBase(void* start_of_memory, size_t size)
    : start_of_memory_(start_of_memory), size_(size) {
...
size_t PermebufBase::AllocateMessage(size_t size) {
  ...
  if (GrowTo(size_ + total_length)) {
    ...
    memset(reinterpret_cast<void*>(reinterpret_cast<size_t>(start_of_memory_) +
                                   current_ptr + size_bytes),
           0, size);
    return current_ptr;
  } else {
...
PermebufMiniMessage::PermebufMiniMessage() {
  for (int word = 0; word < 4; word++) words[word] = 0;
}

void PermebufMiniMessage::Serialize(size_t& a, size_t& b, size_t& c,
                                    size_t& d) const {
  a = words[0];
  b = words[1];
  c = words[2];
...
PermebufService::PermebufService(::perception::ProcessId process_id,
                                 ::perception::MessageId message_id)
    : process_id_(process_id), message_id_(message_id) {}

ProcessId PermebufService::GetProcessId() const { return process_id_; }

MessageId PermebufService::GetMessageId() const { return message_id_; }
...
MessageId PermebufServer::GetMessageId() const { return message_id_; }
