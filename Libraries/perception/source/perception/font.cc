...
int Font::GetHeight() { return kFontHeight; }
...
int Font::MeasureString(std::string_view string) {
  ...
  for (int i = 0; i < string.size(); i++) {
    if (string[i] >= STB_FONT_DejaVuSans_FIRST_CHAR) {
      length +=
          font_chars_[string[i] - STB_FONT_DejaVuSans_FIRST_CHAR].advance_int;
    }
  }
  return length;
}

void Font::DrawString(int x, int y, std::string_view string, uint32 color,
                      uint32* buffer, int buffer_width, int buffer_height) {
  uint8* color_components = (uint8*)&color;
  for (int i = 0; i < string.size(); i++) {
    if (string[i] < STB_FONT_DejaVuSans_FIRST_CHAR ||
        string[i] >=
            STB_FONT_DejaVuSans_NUM_CHARS + STB_FONT_DejaVuSans_FIRST_CHAR)
      continue;

    stb_fontchar* font_char =
        &font_chars_[string[i] - STB_FONT_DejaVuSans_FIRST_CHAR];
        ...
        if (x >= 0 && x < buffer_width && y >= 0 && y < buffer_height) {
          ...
          if (c) {
            if (c == 255) {
              ...
              sc_buf[0] = 0xFF;
              sc_buf[1] = (uint8)((alpha * (int)color_components[1] +
                                   inv_alpha * (int)sc_buf[1]) >>
                                  8);
              sc_buf[2] = (uint8)((alpha * (int)color_components[2] +
                                   inv_alpha * (int)sc_buf[2]) >>
                                  8);
              sc_buf[3] = (uint8)((alpha * (int)color_components[3] +
                                   inv_alpha * (int)sc_buf[3]) >>
                                  8);
            }
          }
        }
...
Font* Font::LoadFont(FontFace font_face) {
  ...
  switch (font_face) {
    case FontFace::DejaVuSans:
      font_bitmap = new uint8[STB_FONT_DejaVuSans_BITMAP_HEIGHT *
                              STB_FONT_DejaVuSans_BITMAP_WIDTH];
      font_chars = new stb_fontchar[STB_FONT_DejaVuSans_NUM_CHARS];
      stb_font_DejaVuSans(
          font_chars, (uint8(*)[STB_FONT_DejaVuSans_BITMAP_WIDTH])font_bitmap,
          kFontHeight);
