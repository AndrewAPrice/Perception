...
void DrawSprite1bitAlpha(int x, int y, uint32 *sprite, int width, int height,
  ...
  if (minx > start_x) start_x = minx;

  if (miny > start_y) start_y = miny;

  int end_x = x + width;
  int end_y = y + height;

  if (end_x > maxx) end_x = maxx;

  if (end_y > maxy) end_y = maxy;
...
void DrawXLine(int x, int y, int width, uint32 colour, uint32 *buffer,
  ...
  for (; x < end_x; x++, indx++) buffer[indx] = colour;
}
...
void DrawYLine(int x, int y, int height, uint32 colour, uint32 *buffer,
  ...
  int indx = buffer_width * y + x;
  for (; y < end_y; y++, indx += buffer_width) buffer[indx] = colour;
}
