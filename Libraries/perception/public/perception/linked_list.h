...
class LinkedList {
 public:
  ...
  ~LinkedList() {
    if (!IsEmpty())
      std::cout << "LinkedList being deallocated while not empty.\n";
  }
  ...
  class Iterator {
   public:
    Iterator(LinkedList* list, C* item, bool forward)
        : list_(list), item_(item), forward_(forward) {}

    C* operator*() const { return item_; }

    Iterator& operator++() {
      item_ = forward_ ? list_->NextItem(item_) : list_->PreviousItem(item_);
      return *this;
    }
  ...
  Iterator begin() { return Iterator(this, FirstItem(), /*forward=*/true); }

  Iterator end() { return Iterator(this, nullptr, /*forward=*/true); }

  Iterator rbegin() { return Iterator(this, LastItem(), /*forward=*/false); }

  Iterator rend() { return Iterator(this, nullptr, /*forward=*/false); }
