...
// Determines the address size. Larger addresses allow the overall Permebuf to
// grow larger, however, data structures take up more memory.
...
class PermebufBase {
 public:
  ...
  template <class T>
  PermebufListOfEnums<T> AllocateListOfEnums() {
    return PermebufListOfEnums<T>(
        this, AllocateMessage(PermebufListOfEnums<T>::AllocateMemory(this)));
  }

  template <class T>
  PermebufListOf<T> AllocateListOf() {
    return PermebufListOf<T>(
        this, AllocateMemory(PermebufListOf<T>::GetSizeInBytes(this)));
  }

  template <class T>
  PermebufListOfOneOfs<T> AllocateListOfOneOfs() {
    return PermebufListOfOneOfs<T>(
        this, AllocateMemory(PermebufListOfOneOfs<T>::GetSizeInBytes(this)));
  }

  PermebufListOfBooleans AllocateListOfBooleans() {
    return PermebufListOfBooleans(
        this, AllocateMemory(PermebufListOfBooleans::GetSizeInBytes(this)));
  }

  PermebufListOfStrings AllocateListOfStrings() {
    return PermebufListOfStrings(
        this, AllocateMemory(PermebufListOfStrings::GetSizeInBytes(this)));
  }

  PermebufListOfBytes AllocateListOfBytes() {
    return PermebufListOfBytes(
        this, AllocateMemory(PermebufListOfBytes::GetSizeInBytes(this)));
  }

  template <class T>
  PermebufListOfNumbers<T> AllocateListOfNumbers() {
    return PermebufListOfNumbers<T>(
        this, AllocateMemory(PermebufListOfNumbers<T>::GetSizeInBytes(this)));
  }
  ...
  size_t GetAddressSizeInBytes() const {
    return 1 << static_cast<size_t>(GetAddressSize());
  }
...
class Permebuf : public PermebufBase {
 public:
  ...
  Permebuf(PermebufAddressSize address_size = PermebufAddressSize::BITS_16)
      : PermebufBase(address_size) {
    // Allocate the first message in the Permebuf.
    root_ = AllocateMessage<T>();
  }

  // Wraps a Permabug around raw memory. This memory must be page aligned, and
  ...
  const T* operator->() const { return &root_; }

  T* operator->() { return &root_; }
...
class PermebufService {
 public:
  PermebufService();
  PermebufService(::perception::ProcessId process_id,
  ...
  bool IsValid() const { return process_id_ != 0; }
 ...
 protected:
  ...
  template <class O>
  ::perception::Status SendMessage(size_t message_id,
                                   Permebuf<O> request) const;

  template <class O, class I>
  StatusOr<I> SendMiniMessageAndWaitForMiniMessage(size_t function_id,
                                                   const O& request) const;

  template <class O, class I>
  StatusOr<Permebuf<I>> SendMiniMessageAndWaitForMessage(
  ...
  template <class O, class I>
  StatusOr<I> SendMessageAndWaitForMiniMessage(size_t function_id,
                                               Permebuf<O> request) const;

  template <class O, class I>
  StatusOr<Permebuf<I>> SendMessageAndWaitForMessage(size_t function_id,
                                                     Permebuf<O> request) const;
...
class PermebufServer {
 ...
 protected:
  ...
  template <class O>
  void ReplyWithStatusOrMessage(::perception::ProcessId process,
                                ::perception::MessageId response_channel,
                                StatusOr<Permebuf<O>> status_or_message);

  void ReplyWithStatus(::perception::ProcessId process,
                       ::perception::MessageId response_channel,
