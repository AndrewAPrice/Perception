...
// implementations and must be part of the header. But, they are in separate
...
template <class T>
PermebufArrayOfEnums<T>::PermebufArrayOfEnums(PermebufBase* buffer,
                                              size_t offset)
...
void PermebufArrayOfEnums<T>::Set(int index, T value) {
  ...
  buffer_->Write2Bytes(first_item_address_ + index * 2,
...
T PermebufArrayOf<T>::Get(int index) const {
  ...
  return T(buffer_, buffer_->ReadPointer(
...
bool PermebufArrayOf<T>::Has(int index) const {
  ...
  return buffer_->ReadPointer(
             first_item_address_ + index
             << static_cast<size_t>(buffer_->GetAddressSize())) != 0;
}
...
void PermebufArrayOf<T>::Set(int index, T value) {
  ...
  buffer_->WritePointer(first_item_address_ + index
                            << static_cast<size_t>(buffer_->GetAddressSize()),
                        value.Address());
}
...
template <class T>
PermebufArrayOfOneOfs<T>::PermebufArrayOfOneOfs(PermebufBase* buffer,
                                                size_t offset)
    : PermebufArray(buffer, offset) {}
...
T PermebufArrayOfOneOfs<T>::Get(int index) const {
  ...
  size_t oneof_offset =
      first_item_address_ + index
      << static_cast<size_t>(buffer_->GetAddressSize()) + index * 2;
...
void PermebufArrayOfOneOfs<T>::Set(int index, T value) {
  ...
  size_t destination_oneof_offset =
      first_item_address_ + index
      << static_cast<size_t>(buffer_->GetAddressSize()) + index * 2;
...
template <class T>
PermebufArrayOfNumbers<T>::PermebufArrayOfNumbers(PermebufBase* buffer,
                                                  size_t offset)
    : PermebufArray(buffer, offset) {}
  ...
  switch (sizeof(T)) {
    case 1:
      return reinterpret_cast<T>(
          buffer_->Read1Byte(first_item_address_ + index));
    case 2:
      return reinterpret_cast<T>(
          buffer_->Read2Bytes(first_item_address_ + index * 2));
    case 4:
      return reinterpret_cast<T>(
          buffer_->Read4Bytes(first_item_address_ + index * 4));
    case 8:
      return reinterpret_cast<T>(
          buffer_->Read8Bytes(first_item_address_ + index * 8));
    default:
      return static_cast<T>(0);
  }
}
...
void PermebufArrayOfNumbers<T>::Set(int index, T value) {
  ...
  switch (sizeof(T)) {
    case 1:
      buffer_->Write1Byte(first_item_address_ + index,
    ...
    case 2:
      ...
      break;
    case 4:
      buffer_->Write4Bytes(first_item_address_ + index * 4,
                           reinterpret_cast<uint32_t>(value));
      break;
    case 8:
      buffer_->Write8Bytes(first_item_address_ + index * 8,
...
void PermebufListOfEnums<T>::Set(T value) {
  ...
  this->buffer_->Write2Bytes(this->GetItemAddress(),
                             static_cast<uint16_t>(value));
}
...
template <class T>
PermebufListOfOneOfs<T>::PermebufListOfOneOfs(PermebufBase* buffer,
                                              size_t offset)
...
T PermebufListOfOneOfs<T>::Get() const {
  if (!this->IsValid()) return T(this->buffer_, 0);
...
template <class T>
PermebufListOfOneOfs<T> PermebufListOfOneOfs<T>::Allocate(
    PermebufBase* buffer) {
  return buffer->AllocateListOfOneOfs<T>();
}

template <class T>
PermebufListOfNumbers<T>::PermebufListOfNumbers(PermebufBase* buffer,
                                                size_t offset)
...
T PermebufListOfNumbers<T>::Get() const {
  ...
  switch (sizeof(T)) {
    case 1:
      return reinterpret_cast<T>(
          this->buffer_->Read1Byte(this->GetItemAddress()));
    case 2:
      return reinterpret_cast<T>(
          this->buffer_->Read2Bytes(this->GetItemAddress()));
    case 4:
      return reinterpret_cast<T>(
          this->buffer_->Read4Bytes(this->GetItemAddress()));
    case 8:
      return reinterpret_cast<T>(
          this->buffer_->Read8Bytes(this->GetItemAddress()));
    default:
      return static_cast<T>(0);
  }
}
...
void PermebufListOfNumbers<T>::Set(T value) {
  ...
  switch (sizeof(T)) {
    case 1:
      this->buffer_->Write1Byte(this->GetItemAddress(),
                                reinterpret_cast<uint8_t>(value));
      break;
    case 2:
      this->buffer_->Write2Byte(this->GetItemAddress(),
                                reinterpret_cast<uint16_t>(value));
      break;
    case 4:
      this->buffer_->Write3Byte(this->GetItemAddress(),
                                reinterpret_cast<uint32_t>(value));
      break;
    case 8:
      this->buffer_->Write1Byte(this->GetItemAddress(),
...
template <class T>
PermebufListOfNumbers<T> PermebufListOfNumbers<T>::Allocate(
    PermebufBase* buffer) {
...
void PermebufService::SendMiniMessageAndNotifyOnMessage(
    size_t function_id, const O& request,
    const std::function<void(StatusOr<Permebuf<I>>)>& on_response) const {
  std::cout
      << "TODO: Implement PermebufService::SendMiniMessageAndNotifyOnMessage"
      << std::endl;
}
...
template <class O, class I>
StatusOr<Permebuf<I>> PermebufService::SendMessageAndWaitForMessage(
    size_t function_id, Permebuf<O> request) const {
  // Send the message.
  void* memory_address;
  size_t number_of_pages;
...
void PermebufService::SendMessageAndNotifyOnMessage(
    size_t function_id, Permebuf<O> request,
    const std::function<void(StatusOr<Permebuf<I>>)>& on_response) const {
  std::cout << "TODO: Implement PermebufService::SendMessageAndNotifyOnMessage"
            << std::endl;
}
...
void PermebufServer::ReplyWithStatusOrMessage(
  ...
  if (status_or_message) {
    ...
    ReplyWithStatus(process, response_channel, status_or_message.Status());
  }
}
