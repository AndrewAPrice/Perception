...
const downloadUrl = 'https://download.adobe.com/pub/adobe/dng/dng_sdk_1_6.zip';

// Check that the third directory exists.
if (!fs.existsSync('third_party/download.zip')) {
	console.log('Downloading Adobe DNG SDK');
	// Grab it from github.
	let command = 'curl ' + downloadUrl + ' --output third_party/download.zip';
	try {
		child_process.execSync(command, {stdio: 'inherit'});
	} catch (exp) {
		console.log('Error downloading Adobe DNK SDK: ' + exp);
		process.exit(1);
	}
}
...
copyFilesInDirectory('third_party/extracted/dng_sdk_1_6/dng_sdk/source', 'source', '.cpp');
copyFilesInDirectory('third_party/extracted/dng_sdk_1_6/dng_sdk/source', 'public', '.h');
