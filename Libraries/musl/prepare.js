...
let destinationFilesBySourceFiles = {};
...
function rename(fromPath, toPath) {
  destinationFilesBySourceFiles[toPath] =
      destinationFilesBySourceFiles[fromPath];
...
replaceInFile(
    'source/env/__libc_start_main.c',
...
replaceInFile(
    'include/unistd.h', 'long syscall(long, ...);',
    '#ifndef syscall\nlong syscall(long, ...);\n#endif');
...
replaceInFile(
    'source/math/fma.c', '#include "../fma.c"', '#include "fma.c.inl"');
replaceInFile(
    'source/math/fmaf.c', '#include "../fmaf.c"', '#include "fmaf.c.inl"');
...
copyFilesInDirectory('third_party/src/setjmp/x86_64', 'source/setjmp/x86_64');
...
// SED alltypes.h
{
  ...
  if (rebuildAllTypes) {
    ...
    const command =
        'sed -f third_party/tools/mkalltypes.sed include/bits/alltypes.h.in ' +
