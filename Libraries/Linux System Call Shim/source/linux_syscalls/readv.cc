...
long readv(long fd, void *iov, long iovcnt) {
	...
	bytes_to_read = std::min(bytes_to_read,
		descriptor->file.size_in_bytes - descriptor->file.offset_in_file);
	...
	for (int chunk_to_read = 0; chunk_to_read < num_chunks; chunk_to_read++) {
		...
		if (!status_or_response) {
			...
			kSharedMemoryPool.ReleaseSharedMemory(std::move(
				pooled_shared_memory));
			errno = EINVAL;
			return -1;
		}
		...
		for (int io_entry = 0; io_entry < iovcnt; io_entry++) {
			...
			size_t buffer_start = descriptor->file.offset_in_file +
				buffer_start_offset;
