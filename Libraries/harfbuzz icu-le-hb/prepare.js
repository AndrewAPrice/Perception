...
const git_repository = 'https://github.com/harfbuzz/icu-le-hb.git';

// Check that the third directory exists.
if (!fs.existsSync('third_party')) {
	console.log('Downloading harfbuzz icu-le-hb');
	// Grab it from github.
	const command = 'git clone ' + git_repository + ' third_party';
	try {
		child_process.execSync(command, {stdio: 'inherit'});
	} catch (exp) {
		console.log('Error downloading harfbuzz icu-le-hb: ' + exp);
		process.exit(1);
	}
} else {
	console.log('Attempting to update harfbuzz icu-le-hb');
	// Try to update it.
	const command = 'git pull ' + git_repository;
	try {
		child_process.execSync(command, {cwd: 'third_party', stdio: 'inherit'});
	} catch (exp) {
		console.log('Error updating harfbuzz icu-le-hb: ' + exp);
		process.exit(1);
	}
}
...
const acceptableExtensions = {'.c':1, '.cpp':1, '.h':1, '.hh':1};
...
copyFilesInDirectory('third_party/src', 'public/layout', '.h');
