...
Status ExecuteReadOnChannel(IdeChannel* channel, IdeRequest* request) {
  ...
  while (true) {
    if (can_do_dma) {
      ...
      auto get_phys_addr = [&](size_t offset_in_buffer) -> size_t {
        if (!request->can_write) {
          size_t relative_offset = offset_in_buffer - request->offset_in_buffer;
          size_t page_index = relative_offset / kPageSize;
          size_t page_offset = relative_offset % kPageSize;
        ...
        uint8* addr = &request->destination_buffer[offset_in_buffer];
        ...
        size_t phys =
            GetPhysicalAddressOfVirtualAddress(page_number * kPageSize);
        if (phys != 0) {
      ...
      while (sectors_already_read < total_sectors_to_read) {
        ...
        for (size_t i = 0; i < chunk_sectors; i++) {
          ...
          if (current_skip_bytes > 0 || copy_size < ATAPI_SECTOR_SIZE) {
            ...
            size_t phys_start = get_phys_addr(current_buffer_offset);
            size_t phys_end =
                get_phys_addr(current_buffer_offset + ATAPI_SECTOR_SIZE - 1);
          ...
          if (needs_scratch) {
            ...
            if (!request->can_write) {
              size_t relative_offset =
                  current_buffer_offset - request->offset_in_buffer;
              size_t page_index = relative_offset / kPageSize;
              size_t page_offset = relative_offset % kPageSize;
